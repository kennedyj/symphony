...
" The default mapping is registered under to <F13> by default, unless the user
" remapped it already (or a mapping exists already for <F13>)
if !exists("no_plugin_maps") && !exists("no_toggle_mouse_maps")
    if !hasmapto('<SID>ToggleMouse()')
        noremap m :call <SID>ToggleMouse()<CR>
        "inoremap <F13> <Esc>:call <SID>ToggleMouse()<CR>i
    endif
endif
